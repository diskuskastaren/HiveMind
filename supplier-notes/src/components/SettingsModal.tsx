...
type SettingsTab = 'ai' | 'recording' | 'teams' | 'data';
...
function CostEstimatePanel({ activeModel }: { activeModel: string }) {
  ...
  return (
    <div className="rounded-lg border border-gray-100 bg-gray-50 p-3 space-y-2.5">
      ...
      {/* Column headers */}
      <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-3 text-[10px] font-medium text-gray-400 uppercase tracking-wide px-2">
        <span>Model</span>
          ...
          return (
            <div
              ...
              className={`grid grid-cols-[1fr_auto_auto_auto] gap-x-3 items-center text-xs rounded px-2 py-1.5 -mx-2 transition-colors ${
                isActive ? 'bg-blue-50 text-blue-800' : 'text-gray-500'
              }`}
            >
              <span className={`truncate ${isActive ? 'font-medium' : ''}`}>{m.label}</span>
              <span className={`font-mono text-right ${isActive ? 'text-blue-500' : 'text-gray-400'}`}>
                ${whisperCost.toFixed(2)}
              </span>
              <span className={`font-mono text-right ${isActive ? 'text-blue-500' : 'text-gray-400'}`}>
                {summaryLabel}
              </span>
              <span className={`font-mono text-right font-semibold ${isActive ? 'text-blue-700' : 'text-gray-600'}`}>
                ${total.toFixed(2)}
              </span>
            </div>
          );
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider pt-2">{children}</h3>;
}
...
export function SettingsModal() {
  ...
  return (
    ...
    >
      ...
      {/* Modal panel */}
      <div className="relative bg-white rounded-xl shadow-2xl flex w-[740px] h-[85vh] overflow-hidden">

        {/* Left nav */}
        <div className="w-48 bg-gray-50 border-r border-gray-200 flex flex-col py-5 flex-shrink-0">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        onBlur={saveApiKey}
                        placeholder="sk-…"
                        className="w-full pr-9 pl-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                      <button
                        type="button"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
                    >
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="flex-1"
                        />
                        <span className="text-sm font-mono font-medium text-gray-700 w-8 text-right">
            ...
            {activeTab === 'teams' && (
              <>
                ...
                <div className="flex items-start justify-between gap-4 py-1">
                  <div>
                    <p className="text-sm font-medium text-gray-700">Teams meeting detection</p>
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 relative w-10 h-6 rounded-full transition-colors ${
                      settings.teamsEnabled ? 'bg-blue-500' : 'bg-gray-300'
                    }`}
                  >
                    <span
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-gray-700">How to set up Teams integration</p>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors w-full"
                >
                ...
                <div className="border border-red-200 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-1.5 text-sm border border-red-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400 placeholder:text-gray-300"
                    />
                    <button
