  ...
  return (
    <div className="border-t border-gray-200 bg-white flex-shrink-0">
      ...
      >
        ...
        <span className="text-xs font-semibold text-gray-700">Follow-ups</span>
        <span className="text-xs text-gray-400">· {isInternal ? 'Internal' : supplier?.name}</span>
        {openCount > 0 && (
          ...
          <div className="px-4 pb-3 space-y-0.5">
            ...
            <div className="flex items-center gap-2 mb-2 pt-1">
              <input
                ...
                className="flex-1 text-sm px-2.5 py-1.5 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-violet-400 bg-gray-50"
              />
              <button
                onClick={handleAdd}
            ...
            {openCount === 0 && resolvedCount === 0 && (
              <p className="text-xs text-gray-400 text-center py-4">
                ...
                <Bookmark className="w-3 h-3 inline text-gray-400" />.
              </p>
            )}
...
function FollowUpRow({ text, resolved, isTask, onResolve, onDelete, onUnflag, onOpenTask }: FollowUpRowProps) {
  return (
    <div className="group flex items-center gap-2 px-1 py-1 rounded-md hover:bg-gray-50 transition-colors">
      ...
      <span
        className={`flex-1 text-sm min-w-0 truncate ${resolved ? 'line-through text-gray-400' : 'text-gray-700'}`}
        title={text}
      >
        {text}
      </span>
