...
export function TranscriptTab() {
  ...
  const handleStop = useCallback(async () => {
    ...
    if (rawText && apiKey && note?.id) {
      ...
      try {
        ...
        updateNote(note.id, {
          transcript: { ...note.transcript!, summary },
        });
      } catch (e: any) {
  ...
  }, [stop, apiKey, note, updateNote]);

  const handleRegenerateSummary = useCallback(async () => {
    if (!note?.transcript?.rawText || !apiKey || !note.id) return;
    setIsSummarizing(true);
    setSummarizeError('');
    try {
      const summary = await summarizeMeetingTranscript(note.transcript.rawText, apiKey);
      updateNote(note.id, { transcript: { ...note.transcript, summary } });
    } catch (e: any) {
      setSummarizeError(e?.message ?? 'Summarization failed');
    }
    setIsSummarizing(false);
  }, [note, apiKey, updateNote]);

  const handleInsertIntoNote = useCallback(() => {
    if (!note?.transcript?.rawText || !note.id) return;
  ...
  const handleClearTranscript = useCallback(() => {
    if (!note?.id) return;
    if (!window.confirm('Clear the transcript for this note?')) return;
    updateNote(note.id, { transcript: undefined });
  }, [note, updateNote]);
  ...
  const hasTranscript = !!note.transcript?.rawText;
  const displayedLive = (note.transcript?.rawText ?? '') + (liveText ? `${liveText}` : '');
        ...
        <div ref={liveScrollRef} className="flex-1 overflow-y-auto p-4">
          {displayedLive ? (
            <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">
              {displayedLive}
              {liveText && (
                <span className="text-gray-400 italic"> {liveText}</span>
              )}
            </p>
          ) : (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-2 border-b border-gray-100 flex items-center gap-2 flex-shrink-0">
        <span className="text-[10px] text-gray-400">
          {note.transcript?.duration != null ? formatDuration(note.transcript.duration) : '–'}
          {note.transcript?.recordedAt
            ? ` · ${format(new Date(note.transcript.recordedAt), 'MMM d, HH:mm')}`
            : ''}
        </span>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {subTab === 'summary' && (
          <>
            ...
            {!isSummarizing && note.transcript?.summary && (
              <div
                className="text-sm leading-relaxed"
                dangerouslySetInnerHTML={renderSummary(note.transcript.summary)}
              />
            )}
            {!isSummarizing && !note.transcript?.summary && (
              <div className="text-center py-6 space-y-2">
                ...
                <p className="text-xs text-gray-400">
                  {apiKey
                    ...
                    : 'Add an OpenAI API key to generate summaries.'}
                </p>
                {apiKey ? (
                  <button
