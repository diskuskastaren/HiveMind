  ...
  if (selectedSupplier !== null) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-[420px] max-h-[70vh] flex flex-col overflow-hidden">
          {/* Header */}
          ...
          <p className="px-5 pt-3 pb-1 text-xs text-gray-400 flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {supplierNotes.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-8">No existing notes</p>
            )}
            {supplierNotes.map((n) => (
              <button
                key={n.id}
                onClick={() => handleStartExisting(selectedSupplier, n.id)}
                className="w-full text-left px-5 py-3.5 border-b border-gray-50 hover:bg-gray-50 transition-colors flex items-center gap-3 group"
              >
                ...
                <Mic className="w-3.5 h-3.5 text-gray-300 group-hover:text-blue-500 flex-shrink-0 transition-colors" />
              </button>
            ))}
          </div>
          ...
          <div className="p-4 border-t border-gray-100 flex-shrink-0">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
            >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-[420px] overflow-hidden">
        ...
        <div className="px-5 py-5 border-b border-gray-100 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-semibold text-gray-900">Teams meeting detected</h2>
            <p className="text-sm text-gray-500 mt-0.5">Where should this meeting be recorded?</p>
          </div>
          <button
            onClick={dismiss}
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          {currentNote && (
            <button
              ...
              className="w-full text-left px-4 py-3.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-xl transition-colors group"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  {currentSupplierName && (
                    <p className="text-xs text-indigo-500 mt-0.5">{currentSupplierName}</p>
                  )}
                </div>
                <Mic className="w-4 h-4 text-indigo-400 group-hover:text-indigo-600 flex-shrink-0 transition-colors" />
              </div>
            </button>
          )}
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              ...
              <ChevronRight
                className={`w-4 h-4 text-gray-300 transition-transform ${supplierListExpanded ? 'rotate-90' : ''}`}
              />
            </button>
            ...
            {supplierListExpanded && (
              <div className="border-t border-gray-100 max-h-56 overflow-y-auto">
                ...
                >
                  ...
                  <span className="text-sm text-gray-700 flex-1">Internal</span>
                  <ChevronRight className="w-3.5 h-3.5 text-gray-300" />
                </button>
              </div>
            )}
